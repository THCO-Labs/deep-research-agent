...
from deep_research.evidence.evidence_hygiene import report_quality_issues
from deep_research.core.schemas import CoverageMatrix, EvidenceCard, ResearchPlan, SourceRecordV2, VerificationResultV2
from deep_research.acquisition.source_limits import MINIMUM_SOURCE_TARGET
from deep_research.evidence.source_validation import anchor_groups_for_question, branch_terms, content_terms, validate_source_content
from deep_research.evidence.text_terms import cjk_char_count, latin_letter_count, preferred_output_language
...
# 0.90 required a near-maximal simultaneous score on word count, heading count, and
# paragraph density -- proxies for volume/structure, not argument quality. Reports that
# failed to clear it externally scored well on both RACE and FACT (e.g. 0.50/0.94), while
# chasing the old bar pushed synthesis toward padding and over-sectioning. Lowered to a
# level clearable by genuinely thorough reports without rewarding padding.
CRITERIA_RICH_REPORT_DEPTH_THRESHOLD = 0.82
...
def verify_report_v2(
    *,
    ...
    if source_breadth_score < 1.0 and not criteria_rich:
        # Source breadth is a soft target for criteria-rich (benchmark) reports: it already
        # does not gate `valid` below, and surfacing it as a repair-round failure nudges the
        # model to cram in more citations at the expense of coherent synthesis.
