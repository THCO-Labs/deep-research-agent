...
from deep_research.synthesis.synthesis_selection import _cards_by_branch, _cards_for_synthesis, _rank_cards, _source_diverse_cards
from deep_research.evidence.text_terms import cjk_char_count
...
def _expand_report_depth_if_needed(
    *,
    ...
    for round_index in range(MAX_DEPTH_EXPANSION_ROUNDS):
        if not _report_needs_depth_expansion(expanded, target_profile, citations_enabled=citations_enabled):
            break
        ...
        expanded = _normalize_report_markdown(expanded, sources, citations_enabled=citations_enabled)
...
- {"Every factual paragraph must include source citations like [3]." if citations_enabled else "Do not insert any citation markers -- a separate pass adds those after this draft is finished."}
- Use only source_id values from the evidence cards above as your factual ground.
...
{"- Repair weak-citation feedback by rewriting unsupported claims or replacing broad citations with claims that the cited evidence cards directly support." if citations_enabled else ""}
- Prefer cohesive analytical prose over bullet dumps. Use tables/Mermaid diagrams/LaTeX where they clarify the analysis.
...
def _report_needs_depth_expansion(
    report: str, target_profile: dict[str, Any], *, citations_enabled: bool = True
) -> bool:
    status = _depth_status(report, target_profile)
    needs_citation_depth = citations_enabled and status["cited_paragraphs"] < status["minimum_cited_paragraphs"]
    return bool(
        status["word_count"] < status["minimum_words"]
        or needs_citation_depth
        ...
        if isinstance(parsed, dict):
            ...
            if not outline:
                logging.getLogger(__name__).warning(
                    "argumentative outline: model returned an empty/non-string mapping: %r", parsed
                )
            return outline
        logging.getLogger(__name__).warning(
            "argumentative outline: model response was not a JSON object: %r", parsed
        )
    except Exception as exc:
        logging.getLogger(__name__).warning("argumentative outline: build failed (%s): %s", type(exc).__name__, exc)
    return {}
