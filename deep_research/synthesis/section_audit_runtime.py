...
from deep_research.synthesis.section_writing import audit_report_sections, refine_section_audits_from_payload
...
def audit_report_sections_with_model(
    *,
    ...
    # Deliberately settings.verifier_model, not _synthesis_model_spec(): the
    # latter unconditionally returns settings.synthesis_model when set, which
    # would silently reroute this verifier-role call onto the writer's model.
    model = model_for_role(settings, "verifier", settings.verifier_model)
    ...
    try:
        response = _invoke_with_synthesis_budget(model, prompt=prompt, settings=settings, model_spec=settings.verifier_model)
