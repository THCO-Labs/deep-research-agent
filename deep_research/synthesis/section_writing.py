...
from deep_research.verification.verifier import parse_inline_citations

# Any exact figure: bare years, thousands-separated numbers, plain multi-digit
# counts, quantities-with-units, and currency. These only need a citation
# *somewhere in the same paragraph*, not in the exact sentence, so paraphrasing a
# date or spreading a fact across sentences no longer trips the audit.
...
SECTION_EVIDENCE_LINKAGE_THRESHOLD = 0.2

# Summary-style sections (the opening answer frame and the cross-source synthesis)
# exist to summarize, not to re-cite every assigned source. Weak evidence linkage
# is a warning for these sections rather than a hard failure.
_SUMMARY_ROLES = {"answer_frame", "cross_source_synthesis"}
_SUMMARY_SECTION_IDS = {"opening_answer", "cross_source_synthesis"}
...
def audit_report_sections(
    *,
    ...
    for section in plan.sections:
        ...
        if draft is None:
            # No heading matched this contract, but the LLM may have folded the
            # branch into a combined section. Audit against the best-overlapping
            # report section rather than flagging the contract as empty.
            draft = _fallback_section_draft(section, drafts)
        audits.append(
            audit_section_draft(
                section=section,
...
def _heading_match(heading: str, sections: list[SectionContract], used_contracts: set[str]) -> SectionContract | None:
    ...
    for section in sections:
        ...
        is_substring = bool(words_heading) and (clean_heading in clean_title or clean_title in clean_heading)
        # A substring match is strong signal even when Jaccard is low (e.g. a long
        # natural heading that contains the short contract title). Give it a floor
        # so it can win selection over a marginal token-overlap match.
        score = max(jaccard, 0.5) if is_substring else jaccard
        if (jaccard >= 0.15 or is_substring) and score > best_score:
            best_score = score
...
def extract_report_sections(
    ...
    *,
    ...
    # Pass 3: Positional fallback -- assign any still-unused contracts to the
    # remaining unmatched blocks in document order. This keeps a planned section
    # from being flagged empty when the writer renamed a heading past recognition
    # or reordered the layout slightly.
    remaining_contracts = [section for section in plan.sections if section.id not in used_contracts]
    leftover_blocks = [i for i in range(len(blocks)) if i not in matched_blocks]
    for block_index, section in zip(leftover_blocks, remaining_contracts):
        used_contracts.add(section.id)
        drafts_dict[block_index] = (section.id, 0.15)
        matched_blocks.add(block_index)

    # Any block still unmatched keeps a slug id derived from its heading.
    for i, (heading, markdown) in enumerate(blocks):
        if i not in drafts_dict:
...
def audit_section_draft(
    *,
    ...
    for paragraph in _paragraphs(markdown):
        # A citation anywhere in the paragraph grounds its figures. This lets a
        # number be stated in one sentence and cited in the next, instead of
        # rejecting the section because the exact sentence lacked a bracket.
        if parse_inline_citations(paragraph):
            continue
        for sentence in _sentences(paragraph):
            if EXACT_CLAIM_RE.search(sentence):
    ...
    if expected_source_ids:
        ...
        if evidence_linkage_score < SECTION_EVIDENCE_LINKAGE_THRESHOLD:
            message = f"Section cites too little of its assigned evidence: {evidence_linkage_score}"
            is_summary = section_id in _SUMMARY_SECTION_IDS or str(section_dict.get("role") or "") in _SUMMARY_ROLES
            if is_summary:
...
def refine_section_audits_from_payload(
    ...
    """Apply an LLM audit review as advisory context on top of deterministic checks.

    Only deterministic checks gate a section. The LLM reviewer's own findings are
    recorded as warnings, never blocking failures, so a subjective critique
    ("could use a more nuanced discussion") can't trap the run in a repair loop.
    """
    if not isinstance(payload, dict):
        return base_payload
    base_audits = {
    ...
    for review in payload.get("section_reviews", []):
        ...
        # Deterministic failures are the only ones that gate; the LLM's findings
        # are downgraded to warnings rather than added to failures.
        audit["failures"] = _dedupe_terms(deterministic_failures)
        audit["warnings"] = _dedupe_terms(
            [str(item) for item in audit.get("warnings", [])] + llm_warnings + llm_failures
        )
        if repair_guidance:
            audit["repair_guidance"] = repair_guidance
        # Lock state is decided by deterministic checks alone. The LLM review can
        # neither block a section that passed nor lock one that failed, so its
        # advisory opinion can't drive a retry.
        audit["locked"] = not audit["failures"]
...
def _fallback_section_draft(section: SectionContract, drafts: list[SectionDraft]) -> SectionDraft | None:
    """Best content-overlap report section for a contract that no heading matched.

    Lets a section that merges two planned branches satisfy both contracts (its
    citations/claims get audited under each) instead of leaving one branch marked
    empty. Deliberately ignores whether a draft is already assigned elsewhere.
    """
    ...
    if not contract_terms:
        return None
    best: tuple[SectionDraft | None, float] = (None, 0.0)
    for draft in drafts:
        if not draft.markdown.strip():
            continue
        haystack_terms = content_terms(f"{draft.heading} {draft.markdown[:2000]}")
        score = len(haystack_terms & contract_terms) / max(len(contract_terms), 1)
        if score > best[1]:
            best = (draft, round(score, 4))
    if best[1] < 0.2:
        return None
    return best[0]
