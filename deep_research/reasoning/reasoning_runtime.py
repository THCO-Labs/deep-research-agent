...
def refine_reasoning_state_with_model(
    *,
    ...
    reflection_log: list[str] | None = None,
) -> dict[str, Any]:
...
def _reasoning_refinement_prompt(
    *,
    ...
    reflection_log = reflection_log or []
    reflection_text = (
        "\n".join(f"- Round {i + 1}: {entry}" for i, entry in enumerate(reflection_log[-5:]))
        if reflection_log
...
Your own rationale from previous rounds (what you decided and why) -- use this to avoid repeating a
search direction that already turned out unproductive, and to recognize when you're genuinely no
longer finding anything new versus when a real gap remains:
{reflection_text}
